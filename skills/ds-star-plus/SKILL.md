...
5. **Finalize** (Haiku, Sonnet if format is complex) to the exact required output format.
   Before responding, run the **format token checklist**:
   - Every `@key[value]` token required by the format spec must appear in the response.
   - Brackets must contain the **actual computed value**, not the placeholder name:
     ✗ `@mean[mean]` (placeholder copied) · ✓ `@mean[34.65]` (real value)
   - Multi-token formats: all tokens must be present — one missing token = wrong answer.
   - If any token is missing or malformed, emit a corrective print and recheck.
   The verifier rubric also checks FORMAT_TOKEN_MISSING as a `fail` condition (see below).
