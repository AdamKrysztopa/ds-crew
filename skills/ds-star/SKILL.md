...
1. **Every required `@key[value]` token is present** — if the format spec lists
...
2. **No placeholder names in brackets** — the format spec writes `@key[variable_name]`
   to show *structure*, not to be copied literally. The bracket must contain the actual
   computed value:
   - ✗ WRONG: `@price_range_mean[mean]` (copied the placeholder name)
   - ✓ RIGHT: `@price_range_mean[16.65]` (actual computed value)

3. **Multi-token formats: emit every token** — if the format requires
   `@mean[...] @median[...] @std_dev[...]`, all three must appear. A single missing
   token marks the entire answer wrong.

If any token is missing or malformed after your finalizer script runs, go back and add
a corrective print statement that emits the token(s) in the right form.
