...
How these skills actually perform on [InfiAgent-DAEval](https://github.com/InfiAgent/InfiAgent)
(18-question DABench dev subset, `claude-sonnet-4-6`):

| variant | strict accuracy | field-score | $/task | avg turns |
|---|---|---|---|---|
| no-tools (text only) | 33% | — | $0.20 | 1 |
| `/ds-star` plugin v1.3.1 | **94.4%** (17/18) | 97.9% | $0.34 | 4.3 |
| ds-star as raw prompt | **94.4%** (17/18) | 97.9% | $0.31 | — |

The single miss (Q8) is a benchmark-label bug — its constraint asks for the population std
while its label uses the sample std. Full study with methodology, charts, the harness/scoring
fixes, and per-question breakdown: [`docs/experiments/`](experiments/README.md).
