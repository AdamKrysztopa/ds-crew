...
class TestRunner(unittest.TestCase):
    ...
    def test_partial_credit_recorded_alongside_strict(self):
        """A multi-field answer that fails strict still records its per-field fraction."""
        questions = [
            {"id": "q1", "question": "...", "files": [], "label": "3.5", "fmt": "stat",
             "hard": False,
             "subquestions": [{"fmt": "stat", "label": "3.5"}, {"fmt": "pval", "label": "0.01"}]},
        ]
        # First field right, second wrong -> strict WRONG, field_score 0.5
        solver = FakeSolver({"q1": {"answer": "@stat[3.5] @pval[99.0]",
                                    "input_tokens": 100, "output_tokens": 10,
                                    "wall_time_s": 1.0, "rounds": 2,
                                    "model_id": "claude-sonnet-4-6"}})
        with tempfile.TemporaryDirectory() as d:
            out = os.path.join(d, "results.jsonl")
            run_variant("v", questions, solver, PRICES, out)
            row = json.loads(open(out).read())
        self.assertFalse(row["correct"])          # strict metric unchanged
        self.assertEqual(row["fields_matched"], 1)
        self.assertEqual(row["fields_total"], 2)
        self.assertEqual(row["field_score"], 0.5)
