...
def summarize(rows):
    ...
    for v in variants:
        ...
        # field_score is a diagnostic; legacy rows without it fall back to strict correctness
        def _field_score(r):
            return r["field_score"] if "field_score" in r else float(r["correct"])
        out[v] = {
            "n": len(rs),
            ...
            "mean_field_score": sum(_field_score(r) for r in rs) / len(rs) if rs else 0.0,
...
def render_summary_md(summary):
    head = "| variant | n | accuracy | accuracy-hard | field-score | $/task | tokens/task | median rounds |"
    sep = "|---|---|---|---|---|---|---|---|"
    lines = [head, sep]
    for v, s in summary.items():
        lines.append(f"| {v} | {s['n']} | {s['accuracy']:.3f} | {s['accuracy_hard']:.3f} "
                     f"| {s['mean_field_score']:.3f} "
